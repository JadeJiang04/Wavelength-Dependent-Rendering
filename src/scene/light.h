...
class DirectionalLight : public SceneLight {
 public:
  ...
  DirectionalLight(const Vector3D rad, const Vector3D lightDir, double temperature);
  Vector3D sample_L(const Vector3D p, Vector3D* wi, double* distToLight,
                    double* pdf) const;
  double sample_L_wavelength(const Vector3D p, Vector3D* wi, double* distToLight,
 ...
 private:
  Vector3D radiance;
  Vector3D dirToLight;
  double light_temperature; // Temperature in Kelvin for black body radiation

  // Helper function to calculate black body radiation intensity
  double black_body_intensity(double temperature, double wavelength) const;
  void calculate_radiance_from_temperature();
...
class PointLight : public SceneLight {
 public: 
  ...
  PointLight(const Vector3D rad, const Vector3D pos, double temperature);
  Vector3D sample_L(const Vector3D p, Vector3D* wi, double* distToLight,
                    double* pdf) const;
  double sample_L_wavelength(const Vector3D p, Vector3D* wi, double* distToLight,
                           double* pdf, double wavelength) const;
