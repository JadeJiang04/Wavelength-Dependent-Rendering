...
BVHNode *BVHAccel::construct_bvh(std::vector<Primitive *>::iterator start,
  ...
  int split_axis = 0;
  for (int i = 1; i <= 2; i++)
    if (bbox.extent[i] > bbox.extent[split_axis])
      split_axis = i;

  double split_point = 0.0f;
  for (auto p = start; p != end; p++) 
    split_point += (*p)->get_bbox().centroid()[split_axis];
  split_point /= size;

  auto mid = std::partition(start, end, [split_axis, split_point](Primitive* prim) {
    return prim->get_bbox().centroid()[split_axis] <= split_point;
  });
...
bool BVHAccel::has_intersection(const Ray &ray, BVHNode *node) const {
  ...
  if (!node->bb.intersect(ray, lb, ub)
   || lb > ray.max_t || ub < ray.min_t) 
    return false;
