...
namespace CGL { namespace SceneObjects {
  ...
  void EnvironmentLight::init() {
    ...
    for (int j = 0; j < h; ++j) {
      ...
      if (row_sum > 0) {
        conds_y[j * w] = pdf_envmap[w * j] / row_sum;
        for (int i = 1; i < w; ++i)
          conds_y[j * w + i] = conds_y[j * w + i - 1] + pdf_envmap[w * j + i] / row_sum;
      } else {
        for (int i = 0; i < w; ++i) 
          conds_y[j * w + i] = (i + 1.0) / w;
      }
    }
  ...
  Vector3D EnvironmentLight::sample_L(const Vector3D p, Vector3D* wi,
    ...
    int j = std::upper_bound(marginal_y, marginal_y + h, sample.y) - marginal_y;
    j = std::min(std::max(j, 0), (int)h - 1);
    
    double* cond_row = conds_y + j * w;
    int i = std::upper_bound(cond_row, cond_row + w, sample.x) - cond_row;
    i = std::min(std::max(i, 0), (int)w - 1);
    ...
    *distToLight = INF_D;
    ...
    if (sin_theta < 1e-8) sin_theta = 1e-8;
    
    *pdf = pdf_envmap[w * j + i] * (w * h) / (2.0 * PI * PI * sin_theta);
