...
namespace CGL {
    ...
    // Helper function for wavelength-dependent BSDF evaluation
    ...
    public:
        ...
        double estimate_direct_lighting_hemisphere_channel(const Ray& r, const SceneObjects::Intersection& isect);
        Vector3D estimate_direct_lighting_importance(const Ray& r, const SceneObjects::Intersection& isect);
        double estimate_direct_lighting_importance_channel(const Ray& r, const SceneObjects::Intersection& isect);

        Vector3D est_radiance_global_illumination(const Ray& r);
        double est_radiance_global_illumination_channel(const Ray& r);
        Vector3D zero_bounce_radiance(const Ray& r, const SceneObjects::Intersection& isect);
        double zero_bounce_radiance_channel(const Ray& r, const SceneObjects::Intersection& isect);
        Vector3D one_bounce_radiance(const Ray& r, const SceneObjects::Intersection& isect);
        double one_bounce_radiance_channel(const Ray& r, const SceneObjects::Intersection& isect);
        Vector3D at_least_one_bounce_radiance(const Ray& r, const SceneObjects::Intersection& isect);
        double at_least_one_bounce_radiance_channel(const Ray& r, const SceneObjects::Intersection& isect);
