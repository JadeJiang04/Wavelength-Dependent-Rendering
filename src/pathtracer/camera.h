...
class Camera {
  ...
  */
  ...
  /**
   * Returns a world-space ray with wavelength-specific sampling
   * \param x x-coordinate of the ray sample in the view plane
   * \param y y-coordinate of the ray sample in the view plane
   * \param color_channel 0=red, 1=green, 2=blue
   */
  Ray generate_ray(double x, double y, int color_channel) const;

  Ray generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const;

  /**
   * Returns a world-space ray with wavelength-specific sampling for thin lens
   * \param x x-coordinate of the ray sample in the view plane
   * \param y y-coordinate of the ray sample in the view plane
   * \param rndR random radius for lens sampling
   * \param rndTheta random angle for lens sampling
   * \param color_channel 0=red, 1=green, 2=blue
   */
  Ray generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta, int color_channel) const;
