...
Ray Camera::generate_ray(double x, double y) const {
  ...
  double halfW = std::tan(hFov * PI / 360.0);
  double halfH = std::tan(vFov * PI / 360.0);

  double new_x = (2.0 * x - 1.0) * halfW;
  double new_y = (2.0 * y - 1.0) * halfH;

  Vector3D sensorC(new_x, new_y, -1.0);
  Vector3D worldC = (c2w * sensorC.unit()).unit();

  return Ray(pos, worldC, fClip);
}

/**
 * This function generates a ray with wavelength-specific sampling from camera perspective
 */
Ray Camera::generate_ray(double x, double y, int color_channel) const {
  double halfW = std::tan(hFov * PI / 360.0);
  double halfH = std::tan(vFov * PI / 360.0);
  ...
  // Red: ~650nm, Green: ~550nm, Blue: ~450nm with sigma ~30nm
  double wavelength;
  ...
  return Ray(pos, worldC, fClip, 0, wavelength);
}
